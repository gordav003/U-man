...
"""Pearsonova korelacija 15-minutnih sprememb U in Q zbiralk 110/SN.
...
Skripta za vsak zvezni casovni segment izdela celotno matriko Pearsonovih
...
Privzeti zagon iz korena projekta:
    ...
    python -m correlations.voltage_reactive_power_110_mv_pearson
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Izracuna Pearsonovo korelacijo tocnih 15-minutnih sprememb "
...
def pearson_cross_matrix(
    du_values: np.ndarray,
    dq_values: np.ndarray,
    min_pair_points: int,
) -> tuple[np.ndarray, np.ndarray]:
    """Pearsonov r za vsak par stolpcev z locenim parnim izlocanjem NaN."""
    valid_u = np.isfinite(du_values)
    valid_q = np.isfinite(dq_values)
    pair_counts = valid_u.astype(np.int32).T @ valid_q.astype(np.int32)
    n_du = du_values.shape[1]
    n_dq = dq_values.shape[1]
    correlation = np.full((n_du, n_dq), np.nan, dtype=float)

    for row in range(n_du):
        for column in range(n_dq):
            valid_pair = valid_u[:, row] & valid_q[:, column]
            if pair_counts[row, column] < min_pair_points:
                continue
            with warnings.catch_warnings():
                warnings.simplefilter("ignore", ConstantInputWarning)
                result = pearsonr(
                    du_values[valid_pair, row],
                    dq_values[valid_pair, column],
                )
            correlation[row, column] = float(result.statistic)
...
def same_busbar_results(
    ...
    return (
        ...
        .with_columns(
            pl.Series("pearson_dU_dQ", diagonal, nan_to_null=True),
            pl.Series("n_skupnih_parov_dU_dQ", counts),
        )
        .with_columns(pl.col("pearson_dU_dQ").abs().alias("abs_pearson_dU_dQ"))
        .sort("abs_pearson_dU_dQ", descending=True, nulls_last=True)
...
def draw_heatmap(
    ...
    ax.set_title(
        "Pearson correlation of 15-minute ΔU and ΔQ changes",
    ...
    colorbar.set_label("Pearson coefficient", color="#20252B")
...
def main() -> None:
    ...
    root_summary = [
        "PEARSONOVA KORELACIJA dU-dQ ZBIRALK 110/SN",
        f"Vhod: {input_path}",
    ...
    for segment in segments:
        ...
        correlation, pair_counts = pearson_cross_matrix(
        ...
        write_matrix_csv(
            segment_dir / "pearson_dU_dQ_korelacijska_matrika.csv",
        ...
        draw_heatmap(
            correlation,
            segment_busbar_names,
            segment_dir / "pearson_heatmap_dU_dQ_15min.png",
            segment_dir / "pearson_heatmap_dU_dQ_15min.svg",
        ...
        summary_lines = [
            "PEARSONOVA KORELACIJA dU-dQ 110/SN - ZVEZNI SEGMENT",
