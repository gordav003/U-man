...
"""Spearmanova korelacija 15-minutnih sprememb U in P zbiralk 110/SN.

Zbiralka je lokacija RTP z vsaj enim strogim transformatorjem 110/SN in z
veljavnimi meritvami U ter P na 110-kV strani. Strogi
...
* P je vsota veljavnih P na 110-kV straneh vseh transformatorjev 110/SN;
* dU(t) = U(t + 15 min) - U(t);
* dP(t) = P(t + 15 min) - P(t).

Skripta za vsak zvezni casovni segment izdela celotno matriko Spearmanovih
korelacij dU vseh zbiralk proti dP vseh lokacij. Posebej shrani tudi diagonalo
matrike, torej korelacijo dU in dP na isti lokaciji. Manjkajoce meritve se ne
interpolirajo, meritve iz locenih segmentov pa se nikoli ne zdruzijo.

Privzeti zagon iz korena projekta:

    python -m correlations.voltage_active_power_110_mv_spearman
...
def default_output_dir() -> Path:
    return (
        project_root()
        / "Uman meritve"
        / "korelacija_U_P_110_SN_spearman"
        / "zvezni_segmenti"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "110-kV napetosti in vsote P za vse stroge zbiralke 110/SN."
        )
    )
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        ...
        help="Najmanj skupnih veljavnih parov dU-dP (privzeto: 20).",
    )
    parser.add_argument(
        "--max-gap-minutes",
        type=int,
...
def load_110_sn_busbars(
    ...
    valid_p = pl.col("P").is_not_null() & pl.col("P").is_finite()
    ...
    eligible_locations = (
        ...
        .agg(
            ...
            pl.col("P_trafo_MW").is_not_null().sum().alias("n_veljavnih_P"),
        )
        .filter(pl.col("n_veljavnih_U") > 0)
        .filter(pl.col("n_veljavnih_P") > 0)
        .select("lokacija_od")
    )
    ...
    busbars = (
        ...
        .agg(
            ...
            pl.col("P_trafo_MW").sum().alias("P_110_SN_MW"),
            pl.col("U_trafo_kV").is_not_null().sum().alias("n_trafov_U"),
            pl.col("P_trafo_MW").is_not_null().sum().alias("n_trafov_P"),
        )
        .with_columns(
            pl.when(pl.col("n_trafov_P") > 0)
            .then(pl.col("P_110_SN_MW"))
            .otherwise(None)
            .alias("P_110_SN_MW"),
            (
                pl.col("lokacija_od")
...
def eligible_busbars_for_segment(
    ...
    """Vrne zbiralke z dovolj veljavnimi dU in dP v danem segmentu."""
    availability = (
        changes.group_by("zbiralka")
        .agg(
            pl.col("dU_kV").is_finite().sum().alias("n_veljavnih_dU"),
            pl.col("dP_MW").is_finite().sum().alias("n_veljavnih_dP"),
        )
        .filter(pl.col("n_veljavnih_dU") >= min_pair_points)
        .filter(pl.col("n_veljavnih_dP") >= min_pair_points)
        .select("zbiralka")
    )
...
def spearman_cross_matrix(
    ...
    valid_p = np.isfinite(dp_values)
    pair_counts = valid_u.astype(np.int32).T @ valid_p.astype(np.int32)
    n_busbars = du_values.shape[1]
    combined = np.column_stack([du_values, dp_values])
...
def write_matrix_csv(
    ...
    with path.open("w", encoding="utf-8-sig", newline="") as handle:
        ...
        writer.writerow(["dU_zbiralka", *[f"dP: {name}" for name in busbars]])
...
def build_segment_metadata(
    ...
    measurement_summary = segment_busbars.group_by("zbiralka").agg(
        ...
        pl.col("P_110_SN_MW").is_not_null().sum().alias("n_tock_P"),
        pl.col("n_trafov_U").max().alias("max_trafov_U_v_casu"),
        pl.col("n_trafov_P").max().alias("max_trafov_P_v_casu"),
    ...
    change_summary = changes.group_by("zbiralka").agg(
        ...
        pl.col("dP_MW").is_not_null().sum().alias("n_tocnih_dP_15min"),
        pl.col("dU_kV").n_unique().alias("n_razlicnih_dU"),
        pl.col("dP_MW").n_unique().alias("n_razlicnih_dP"),
    )
    return (
        ...
        .with_columns(
            ...
            pl.col("n_tock_P").fill_null(0),
            pl.col("n_tocnih_dU_15min").fill_null(0),
            pl.col("n_tocnih_dP_15min").fill_null(0),
            pl.col("n_razlicnih_dU").fill_null(0),
            pl.col("n_razlicnih_dP").fill_null(0),
        )
        .sort("lokacija_od")
    )
...
def same_busbar_results(
    ...
    return (
        ...
        .with_columns(
            pl.Series("spearman_dU_dP", diagonal, nan_to_null=True),
            pl.Series("n_skupnih_parov_dU_dP", counts),
        )
        .with_columns(pl.col("spearman_dU_dP").abs().alias("abs_spearman_dU_dP"))
        .sort("abs_spearman_dU_dP", descending=True, nulls_last=True)
...
def draw_heatmap(
    ...
    ax.set_title(
        "Spearman correlation of 15-minute ΔU and ΔP changes",
    ...
    ax.text(
        0,
        1.012,
        "dU(t) = U(t + 15 min) - U(t); dP(t) = P(t + 15 min) - P(t); "
...
def main() -> None:
    ...
    if not busbars:
        raise ValueError(
            "V vhodnih podatkih ni zbiralk 110/SN z veljavnimi meritvami U in P."
        )
    ...
    root_summary = [
        "SPEARMANOVA KORELACIJA dU-dP ZBIRALK 110/SN",
        f"Vhod: {input_path}",
        ...
        "P lokacije: vsota P na 110-kV straneh trafov 110/SN",
        f"Definicija: dU in dP sta tocni {DELTA_MINUTES}-minutni spremembi",
    ...
    for segment in segments:
        ...
        if not segment_busbar_names:
            (segment_dir / "izracun_povzetek.txt").write_text(
                "V segmentu ni zbiralk z dovolj veljavnimi meritvami U in P.\n",
                encoding="utf-8",
            )
            print(
                f"Segment {segment.number}/{len(segments)} preskocen: "
                "ni zbiralk z dovolj veljavnimi U in P."
            )
            continue
        ...
        dp_values = wide_values(changes, segment_busbar_names, "dP_MW")
        correlation, pair_counts = spearman_cross_matrix(
            du_values, dp_values, args.min_pair_points
        )
        ...
        write_matrix_csv(
            segment_dir / "spearman_dU_dP_korelacijska_matrika.csv",
            segment_busbar_names,
            correlation,
        )
        write_matrix_csv(
            segment_dir / "stevilo_skupnih_15min_parov_dU_dP.csv",
            segment_busbar_names,
            pair_counts,
            integer=True,
        )
        ...
        same_location.write_csv(
            segment_dir / "korelacija_dU_dP_iste_zbiralke.csv",
            separator=";",
            include_bom=True,
        )
        draw_heatmap(
            correlation,
            segment_busbar_names,
            segment_dir / "spearman_heatmap_dU_dP_15min.png",
            segment_dir / "spearman_heatmap_dU_dP_15min.svg",
        ...
        summary_lines = [
            "SPEARMANOVA KORELACIJA dU-dP 110/SN - ZVEZNI SEGMENT",
            f"Segment: {segment.label}",
            f"Casovnih tock segmenta: {segment.n_timestamps}",
            f"Stevilo zbiralk/lokacij z veljavnimi U in P: {len(segment_busbar_names)}",
            ...
            "Agregacija U: mediana trafov; agregacija P: vsota trafov",
