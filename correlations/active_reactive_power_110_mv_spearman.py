...
"""Spearmanova korelacija 15-minutnih sprememb P in Q zbiralk 110/SN.

Zbiralka je lokacija RTP z vsaj enim strogim transformatorjem 110/SN in z
veljavnimi meritvami P ter Q na 110-kV strani. Strogi
...
korelacij dP vseh zbiralk proti dQ vseh lokacij. Posebej shrani tudi diagonalo
matrike, torej korelacijo dP in dQ na isti lokaciji. Manjkajoce meritve se ne
interpolirajo, meritve iz locenih segmentov pa se nikoli ne zdruzijo.

Privzeti zagon iz korena projekta:

    python -m correlations.active_reactive_power_110_mv_spearman
...
def default_output_dir() -> Path:
    return (
        project_root()
        / "Uman meritve"
        / "korelacija_P_Q_110_SN_spearman"
        / "zvezni_segmenti"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "vsot P in Q za vse stroge zbiralke 110/SN."
        )
    )
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        ...
        help="Najmanj skupnih veljavnih parov dP-dQ (privzeto: 20).",
    )
    parser.add_argument(
        "--max-gap-minutes",
        type=int,
...
def load_110_sn_busbars(
    ...
    # nimajo pa nobene uporabne 110-kV meritve P ali Q. Dodatni filter po
...
def exact_15_minute_changes(busbars: pl.DataFrame) -> pl.DataFrame:
    ...
    return (
        ...
        .with_columns(
            (pl.col("P_t_plus_15_MW") - pl.col("P_t_MW")).alias("dP_MW"),
            (pl.col("Q_t_plus_15_MVAr") - pl.col("Q_t_MVAr")).alias(
                "dQ_MVAr"
            ),
        )
        .select("time", "zbiralka", "dP_MW", "dQ_MVAr")
        .sort(["time", "zbiralka"])
    )
...
def eligible_busbars_for_segment(
    ...
    """Vrne zbiralke z dovolj veljavnimi dP in dQ v danem segmentu."""
    availability = (
...
def spearman_cross_matrix(
    ...
    pair_counts = valid_p.astype(np.int32).T @ valid_q.astype(np.int32)
    n_busbars = dp_values.shape[1]
    combined = np.column_stack([dp_values, dq_values])
...
def write_matrix_csv(
    ...
    with path.open("w", encoding="utf-8-sig", newline="") as handle:
        ...
        writer.writerow(["dP_zbiralka", *[f"dQ: {name}" for name in busbars]])
        for name, row in zip(busbars, matrix):
            ...
            writer.writerow([f"dP: {name}", *values])
...
def same_busbar_results(
    ...
    return (
        ...
        .with_columns(
            pl.Series("spearman_dP_dQ", diagonal, nan_to_null=True),
            pl.Series("n_skupnih_parov_dP_dQ", counts),
        )
        .with_columns(pl.col("spearman_dP_dQ").abs().alias("abs_spearman_dP_dQ"))
        .sort("abs_spearman_dP_dQ", descending=True, nulls_last=True)
...
def main() -> None:
    ...
    if not busbars:
        raise ValueError(
            "V vhodnih podatkih ni zbiralk 110/SN z veljavnimi meritvami P in Q."
        )
    ...
    root_summary = [
        "SPEARMANOVA KORELACIJA dP-dQ ZBIRALK 110/SN",
        f"Vhod: {input_path}",
        ...
        f"Definicija: dP in dQ sta tocni {DELTA_MINUTES}-minutni spremembi",
    ...
    for segment in segments:
        ...
        if not segment_busbar_names:
            (segment_dir / "izracun_povzetek.txt").write_text(
                "V segmentu ni zbiralk z dovolj veljavnimi meritvami P in Q.\n",
                encoding="utf-8",
            )
            print(
                f"Segment {segment.number}/{len(segments)} preskocen: "
                "ni zbiralk z dovolj veljavnimi P in Q."
            )
            continue
        ...
        correlation, pair_counts = spearman_cross_matrix(
            dp_values, dq_values, args.min_pair_points
        )
        ...
        write_matrix_csv(
            segment_dir / "spearman_dP_dQ_korelacijska_matrika.csv",
            segment_busbar_names,
            correlation,
        )
        write_matrix_csv(
            segment_dir / "stevilo_skupnih_15min_parov_dP_dQ.csv",
            segment_busbar_names,
            pair_counts,
            integer=True,
        )
        ...
        same_location.write_csv(
            segment_dir / "korelacija_dP_dQ_iste_zbiralke.csv",
            separator=";",
            include_bom=True,
        )
        draw_heatmap(
            correlation,
            segment_busbar_names,
            segment_dir / "spearman_heatmap_dP_dQ_15min.png",
            segment_dir / "spearman_heatmap_dP_dQ_15min.svg",
        ...
        summary_lines = [
            "SPEARMANOVA KORELACIJA dP-dQ 110/SN - ZVEZNI SEGMENT",
            f"Segment: {segment.label}",
            f"Casovnih tock segmenta: {segment.n_timestamps}",
            f"Stevilo zbiralk/lokacij z veljavnimi P in Q: {len(segment_busbar_names)}",
            ...
            "Agregacija P in Q: vsota veljavnih meritev trafov",
