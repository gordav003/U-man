...
This repository supports research conducted in connection with the CRESYM U-MAN project, which investigates coordination between transmission system operators (TSOs) and distribution system operators (DSOs) for voltage control and reactive power management.
...
Which substations and network areas contribute most strongly to capacitive reactive power exchange?

Under which loading and generation conditions do critical voltage states occur?

How strongly are voltage variations correlated with active and reactive power flows?

How does operation of the medium-voltage network affect voltage and reactive power conditions at 110 kV?

Which operating states are suitable as representative cases for detailed simulation and control studies?

Which measurements and information exchanges are required for effective TSO–DSO coordination?

Which control actions should remain local, and which require system-level coordination?
...
measurements/prepare_*.py

Normalize SCADA P/Q/U measurements and transformer tap positions.

Voltage analysis

voltage/*.py

Analyze voltage levels, high-voltage events, annual duration curves, and shared voltage data.

Reactive power

reactive_power/*.py

Analyze reactive-power behavior, peaks, annual duration, and compensation scenarios.

Correlation

correlations/*.py, continuous_segments.py

Calculate segmented P/Q/U and cross-voltage-level correlations.

Interactive plotting

parquet_plotter.py

Build and export plots from prepared Parquet components.

Network model

powerfactory_to_pandapower_jacobian.py

Convert a PowerFactory model, solve or reconstruct the operating point, and export the sparse classical AC Jacobian.
...
Interactive Substation Reactive Power Compensation

To compare the original operating state with added capacitive or inductive reactive power,
run:

python -m reactive_power.reactive_power_compensation_gui

In the application window, select the component_files directory and the substation, enter the capacitive or
inductive reactive power in MVAr, and click Calculate and plot. The following sign convention is used:
Q_new = Q_measured + Q_inductive - Q_capacitive. The plot can be saved
as SVG, PNG, or PDF, while results for individual measurement points can be exported to CSV.

If the data are stored in another directory, select it in the application or provide it at startup:
...
To interactively compose plots from prepared components, run:

python parquet_plotter.py

In the application window, find a component (transformer, transmission line, generator, measurement point, or
TAP), select the measurement, define the time period, and add it to the collection. Each item in the collection
may use a different component, measurement, and time period. The collection can be plotted in a single window,
saved as PNG, SVG, or PDF, and the selection can be stored as JSON for reuse.
...
Test data loading without opening the graphical interface:

python parquet_plotter.py --smoke-test

Q–U Correlation by Continuous Segments

The correlation between changes in reactive power and voltage is calculated separately for each continuous
15-minute measurement segment. A time gap always starts a new segment; therefore, data before
and after a gap are not combined into the same correlation coefficient or regression.
...
The main CSV file is an index of all detected segments. For each sufficiently long and
variable segment, a dedicated directory is created containing measurements, changes, statistics,
and plots. Segments that are too short or constant remain documented as
skipped.

Code Organization

voltage/voltage_data.py contains shared element discovery, topology handling, and voltage-series reading
for voltage analyses.
...
measurements/prepare_tap_measurements.py handles
discrete tap-changer positions. They are intentionally separated because they use different
rules for detection, validation, and export.

All dependencies are listed in a single requirements.txt file.

Development and Contributing

Rules for branches, commits, pull requests, and basic checks are described in
CONTRIBUTING.md. The main branch is the only permanent branch;
